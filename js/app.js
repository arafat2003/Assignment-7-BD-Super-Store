...
const showProducts = (products) => {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <span style="margin-right: 10px;">Ratings: ${product.rating.rate}</span>
      <span>Avg: ${product.rating.count}</span>
      <br>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn">add to cart</button>
      <button onclick="loadSingleItem('${product.id}')" id="details-btn" class="btn btn-danger">Details</button></div>
      `;
...
// load single item
const loadSingleItem = singleID => {
    const url = `https://fakestoreapi.com/products/${singleID}`;
    fetch(url)
        .then((response) => response.json())
        .then((data) => singleItemDetails(data));
}

// display single item's detail
const singleItemDetails = product => {
    const div = document.getElementById('div');
    div.innerHTML = `
    <h3 style="font-size: 30px; font-weight: 700;">${product.title}</h3>
    <h4 style="font-size: 25px; font-weight: 400;">Category: ${product.category}</h4>
    <span style="margin-right: 10px; font-size: 18px;">Ratings: ${product.rating.rate}</span>
    <span style="font-size: 18px;">Avg: ${product.rating.count}</span>
    `;
}
loadProducts();

// Didn't load the image and product description in the details area as the cart goes downward according to the space taken by the details area and sometimes fully disappears.



{/*
    <img src="${product.image}" alt="">
    <h4 class="text">${product.rating.rate}</h4> */}
